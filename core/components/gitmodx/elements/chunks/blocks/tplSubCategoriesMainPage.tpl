...
{var $file_exists = $_modx->runSnippet('ifFileExist',['filename' => "{$icon | replace:'/ru/files/':'/files/'}"])}
{if $file_exists?}
    {set $picture = $icon}
{else}
    {set $picture = $img}
...
<li data-aos="zoom-in-up">
  <div class="inner">
    <a href="{$id | url}" class="img">
      <img src="{$_modx->runSnippet('phpthumbon', ['input' => "{$picture | replace:'/ru/files/':'/files/'}", 'options' => 'w=305&h=160&zc=0&bg=ffffff&f=webp'])}" alt="{$pagetitle}" width="305" height="160">
      <span class="number">
