...
$_options_number = array();
$_options_number_key = array();
$_options_number_key_1 = array();
$_options_number_key_slider = array();
$_options_number_key_number = array();
$_options = array();
...
foreach ($options as $option) {
  if ($option['type'] == 'numberfield') {
    $_options_number_key[]  = "tplFilter.outer.msoption|".$option['key'];
    $_options_number_key_1[]  = "tplFilter.row.msoption|".$option['key'];
  }
}

for ($i = 0; $i<count($_options_number_key); $i++) {
  $_options_number_key_slider[$_options_number_key[$i]] = 'tplMfilter2Slider';
}
for ($i = 0; $i<count($_options_number_key_1); $i++) {
  $_options_number_key_number[$_options_number_key_1[$i]] = 'tplMfilter2Number';
}

if ($_options_number_key_slider) {
  $scriptProperties[] =  array_merge($scriptProperties, $_options_number_key_slider);
}

if ($_options_number_key_number) {
  $scriptProperties[] =  array_merge($scriptProperties, $_options_number_key_number);
}
*/
...
$modx->regClientStartupScript('<script ENGINE="text/javascript">mse2Config = ' . json_encode($config) . ';</script>', true);
$log = '';
